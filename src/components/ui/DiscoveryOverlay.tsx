"use client";

import { useSyncExternalStore } from "react";
import { AnimatePresence, motion } from "framer-motion";
import { discoveryStore } from "@/components/world/discoveryStore";
import { profile } from "@/data/profile";
import { careerJourney } from "@/data/career";
import { notalk, todaypop, type Project } from "@/data/projects";
import { business, future, ending } from "@/data/story";

/**
 * 발견 콘텐츠 오버레이 — 카드가 아니라 "여백 위에 떠오르는 텍스트".
 * 또또가 장소/오브젝트에 다가가거나 클릭했을 때만 부드럽게 나타난다.
 * about / career-<i> / notalk / todaypop / business / future / ending
 */
const fade = {
  initial: { opacity: 0, y: 24 },
  animate: { opacity: 1, y: 0 },
  exit: { opacity: 0, y: -16 },
  transition: { duration: 0.8, ease: "easeOut" as const },
};

export function DiscoveryOverlay() {
  const active = useSyncExternalStore(
    discoveryStore.subscribe,
    discoveryStore.getSnapshot,
    () => null,
  );

  const left = active != null && active.startsWith("career-");
  const center = active === "ending";
  const justify = left
    ? "justify-start"
    : center
      ? "justify-center"
      : "justify-end";

  return (
    <div
      className={`pointer-events-none fixed inset-0 z-20 flex items-center ${justify}`}
    >
      <AnimatePresence mode="wait">
        {active === "about" && (
          <motion.div key="about" {...fade} className="max-w-md px-10 text-left md:pr-24">
            <Label tone="text-forest/80">{profile.name.toUpperCase()} · ABOUT</Label>
            <Title>{profile.tagline}</Title>
            <p className="mt-5 max-w-sm leading-relaxed text-ink-soft">{profile.intro}</p>
            <p className="mt-6 font-display italic text-forest">“{profile.philosophy}”</p>
          </motion.div>
        )}

        {left && (
          <CareerCard key={active} index={Number(active.split("-")[1])} />
        )}

        {active === "notalk" && <ProjectView key="notalk" p={notalk} accent="#7fe0d2" />}
        {active === "todaypop" && <ProjectView key="todaypop" p={todaypop} accent="#f6859b" />}

        {active === "business" && (
          <StoryView key="business" data={business} accent="#9fb4e0" />
        )}
        {active === "future" && (
          <StoryView key="future" data={future} accent="#fff0b0" />
        )}
        {active === "ending" && (
          <motion.div key="ending" {...fade} className="max-w-xl px-10 text-center">
            <Label tone="text-[#ffd98a]">{ending.label}</Label>
            <p className="mt-4 font-display text-3xl leading-snug text-cream md:text-4xl">
              {ending.title}
            </p>
            <div className="mt-5 space-y-1 leading-relaxed text-cream/85">
              {ending.lines.map((l, i) => (
                <p key={i}>{l}</p>
              ))}
            </div>
            <p className="mt-7 font-display text-lg text-[#ffe6a8]">{ending.contact}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  );
}

function Label({ children, tone }: { children: React.ReactNode; tone: string }) {
  return (
    <p className={`font-display text-sm tracking-[0.35em] ${tone}`}>{children}</p>
  );
}
function Title({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-4 font-display text-3xl leading-snug text-ink md:text-4xl">
      {children}
    </p>
  );
}

function CareerCard({ index }: { index: number }) {
  const node = careerJourney[index];
  if (!node) return null;
  return (
    <motion.div {...fade} className="max-w-sm px-10 text-left md:pl-24">
      <Label tone="text-forest/80">CAREER · {index + 1}/{careerJourney.length}</Label>
      <p className="mt-3 text-5xl">{node.emoji}</p>
      <p className="mt-2 font-display text-3xl text-ink">{node.company}</p>
      <p className="font-display text-lg text-forest">{node.role}</p>
      {node.desc && (
        <p className="mt-3 max-w-xs leading-relaxed text-ink-soft">{node.desc}</p>
      )}
    </motion.div>
  );
}

function ProjectView({ p, accent }: { p: Project; accent: string }) {
  return (
    <motion.div {...fade} className="max-w-lg px-10 text-left md:pr-24">
      <Label tone="">{p.emoji} PROJECT · {p.title}</Label>
      <Title>{p.tagline}</Title>
      <div className="mt-5 space-y-2 text-sm leading-relaxed text-ink-soft">
        <p><span className="font-display text-ink">문제 </span>{p.problem}</p>
        <p><span className="font-display text-ink">해결 </span>{p.solution}</p>
        <p><span className="font-display text-ink">성과 </span>{p.result}</p>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        {p.stack.map((s) => (
          <span
            key={s}
            className="rounded-full px-3 py-1 text-xs text-ink"
            style={{ backgroundColor: `${accent}40` }}
          >
            {s}
          </span>
        ))}
      </div>
    </motion.div>
  );
}

function StoryView({
  data,
  accent,
}: {
  data: { emoji: string; label: string; title: string; lines: readonly string[] };
  accent: string;
}) {
  return (
    <motion.div {...fade} className="max-w-lg px-10 text-left md:pr-24">
      <p className="font-display text-sm tracking-[0.35em]" style={{ color: accent }}>
        {data.emoji} {data.label}
      </p>
      <p className="mt-4 font-display text-3xl leading-snug text-cream md:text-4xl">
        {data.title}
      </p>
      <div className="mt-5 space-y-1 leading-relaxed text-cream/85">
        {data.lines.map((l, i) => (
          <p key={i}>{l}</p>
        ))}
      </div>
    </motion.div>
  );
}
